...
angular.module('module')
  .controller('ModuleCrl', function ($scope, $modal, httpService, $rootScope) {
    ...
    $scope.querySystemInfoList = function () {
      httpService.req("GET","/mdm/module/moduleInfos", $scope.queryParams, function (result) {
    ...
    $scope.addConfirm = function () {
      ...
      httpService.req("POST","/mdm/module/moduleInfos",{},function (data) {
        if (data && data.data == 1) {
    ...
    $scope.editConfirm = function () {
      ...
      httpService.req("PUT","/mdm/module/moduleInfos/"+$scope.editParams.id, {}, function (data) {
    ...
    $scope.doDel = function (index) {
      layer.confirm('确认要删除当前节点及其所有子节点吗？', {
    	    ...
    	    title:"确定"
		      ...
		      layer.close(flag);
		      httpService.req("DELETE","/mdm/module/moduleInfos/"+$scope.systemInfoList[index].id, {path:$scope.systemInfoList[index].path}, function (data) {
	        if (data && data.data > 0) {
	          $rootScope.addAlert("success", "操作成功!");
	          $scope.querySystemInfoList();
	          $scope.queryMenuList();
