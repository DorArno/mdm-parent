		...
		.controller(
				...
				function($scope, $modal, httpService, $rootScope) {
					...
					$scope.queryEventHandler = function() {
						...
						httpService.req("GET", "/mdm/extraSystemApiConfig/extraSystemApiInfos",
					...
					$scope.querySystemInfoList = function() {
						...
						httpService
								.req(
										"GET",
										"/mdm/extraSystemApiConfig/extraSystemApiInfos",
										$scope.queryParams,
										function(result) {
											var data = result.data;
											if (data != null) {
					...
					$scope.deleteHandler = function() {
						...
						if (confirm('确认删除吗?')) {
							httpService
									.req(
											"POST",
											"/mdm/extraSystemApiConfig/deletExtraSystemApiInfos",
											{},
											function(data) {
												if (data.data == 1) {
													$rootScope.addAlert(
					...
					$scope.initSystemInfo = function(){
					      httpService.req("GET","/mdm/invokeSystem/systemInfos", {}, function (result) {
					          if (result.data) {
					...
					$scope.addTask = function() {
						...
						httpService
								.req(
										"POST",
										"/mdm/extraSystemApiConfig/addExtraSystemApiInfos",
										{}, function(data) {
					...
					$scope.submitEditHandler = function(){
						...
						.req(
								"POST",
								"/mdm/extraSystemApiConfig/editExtraSystemApiInfos",
								{}, function(data) {
