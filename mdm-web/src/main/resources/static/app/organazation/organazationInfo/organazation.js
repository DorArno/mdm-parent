...
//												tree.expandAll(true);
					...
					$scope.queryOrganazationList = function() {
						httpService.req("GET", "/mdm/organazation",
...
//					展示树子节点
					$scope.queryTreeNodes = function(node_id) {
						if (typeof (tree) != "undefined") {
							...
							if("0" != node_id){
								...
								httpService.req('GET', '/mdm/tree/dynamicNodes', params,
									function(result) {
					...
					$scope.addOrganazationInfo = function() {
						...
						httpService.req("GET",
								"/mdm/organazation/" + $scope.addParams.parentId,
								{},
								function(result) {
									$scope.addParams.corpCode = result.data.corpCode;
									//如果你节点是管理处，不能新增新的组织
									if(result.data.idOrganizationType=='PropertyManagementOffice'){
										$rootScope.addAlert("error", "管理处无法新增下级组织！");
										return;
									}
		...
		//						$scope.addParams.typeName = $scope.TypeName[$scope.addParams.type];
					...
					$scope.createRandomCode = function() {
						httpService.req("GET",
								"/mdm/organazation/createRandomCode",
								$scope.queryParams, function(result) {
...
//						httpService.req("GET", "/mdm/invokeSystem/systemInfos",param, function(result) {
...
//					};
					...
					$scope.queryCommunityList = function(districtID) {
						...
						httpService.req("GET", "/mdm/community", params,
								function(result) {
					...
					$scope.addConfirm = function() {
						...
						httpService.req("POST", "/mdm/organazation", {},
								function(result) {
					...
					$scope.editConfirm = function() {
						...
						httpService.req("PUT", "/mdm/organazation/"
				    ...
				    $scope.batchUpdateOrgStatus = function (status) {
				    	...
				    	param.flag = status;
				      	httpService.req("PUT","/mdm/organazation/batchUpdateOrgStatus", {}, function (data) {
				        	if (data.result == "true") {
				          		$rootScope.addAlert("success", "操作成功!");
				          		$scope.queryOrganazationList2();
				        	}
