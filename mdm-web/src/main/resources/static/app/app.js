...
//        	authObj.sysCode = "MDM";
//        	alert(config.url);
//        	httpService.req("POST","/mdm/invokeSystem/systemInfo/code"+"?timeStamp="+timeStamp, {}, function (result) {
//        		alert(result.data);
...
////        	 $http({
////        	      method: "POST",
////        	      url: "/mdm/invokeSystem/systemInfo/code"+"?timeStamp="+timeStamp,
////        	      params: {},
////        	      data: config["data"],
////        	      timeout: 20000
...
//            config.headers.authentication = JSON.stringify(authObj);
        ...
        response: function(response) {
        	...
        	if(response.data.toString().indexOf("/sso/css")>=0){
        		window.location.href="/mdm/test.html";
        		return null;
        	}
        	//debugger;
            return response;
...
angular.module("mdm").directive('roleLevel',function($rootScope, httpService) {
	return {
        ...
        link : function(scope,elem,attr) {
        	httpService.req("GET","/mdm/role/queryRoleLevel", {tableName:attr.table}, function (result) {
    			var html = [];
...
angular.module("mdm").directive('staticData',function($rootScope, httpService) {
	return {
		...
		replace : true,
        link : function(scope,elem,attr) {
        	httpService.req("GET","/mdm/staticdata", {tableName:attr.table}, function (result) {
    			var html = [];
...
function queryUserOperation(httpService,callback) {
	...
	if(returnData == ''){
		httpService.req("GET","/mdm/global/userOperation", {}, function(result) {
			if (result.data) {
