...
angular.module('invokeSystem')
  .controller('SystemInfoCrl', function ($scope, $modal, httpService, $rootScope) {
    ...
    $scope.querySystemInfoList = function () {
      httpService.req("GET","/mdm/invokeSystem/systemInfo", $scope.queryParams, function (result) {
    ...
    $scope.addConfirm = function () {
      ...
      httpService.req("POST","/mdm/invokeSystem/systemInfo",{},function (res) {
        if (res.data == 1) {
...
//      $scope.editParams = $scope.systemInfoList[index];
    ...
    $scope.editConfirm = function () {
    	...
    	layer.confirm('确认要提交？', {
    	    ...
    	    title:"确认"
		      ...
		      httpService.req("PUT","/mdm/invokeSystem/systemInfo/"+$scope.editParams.id, {}, function (res) {
    ...
    $scope.queryWhiteInfoList = function () {
    	 httpService.req("GET","/mdm/invokeSystem/systemInfo/"+$scope.goWhiteParams.id+"/whiteInfo", {}, function (result) {
   	      var data = result.data;
   	      if (data != null) {
    ...
    $scope.addWhiteInfo = function () {
    	...
    	layer.confirm('确认要新增此ip白名单？', {
    	    ...
    	    title:"确认"
		      ...
		      httpService.req("POST","/mdm/invokeSystem/systemInfo/"+$scope.goWhiteParams.id+"/whiteInfo",{},function (res) {
		          if (res.data == 1) {
		            $rootScope.addAlert("success", "操作成功");
		            $scope.queryWhiteInfoList();
		          }
    ...
    $scope.deleteWhiteInfo = function (index) {
        ...
        layer.confirm('确认要删除此ip白名单？', {
    	    ...
    	    title:"确认"
		      ...
		      httpService.req("PUT","/mdm/invokeSystem/systemInfo/whiteInfo/"+$scope.whiteInfoParams.id,{},function (res) {
	          if (res.data == 1) {
	            $rootScope.addAlert("success", "操作成功");
	            $scope.queryWhiteInfoList();
	          }
    ...
    $scope.doDel = function (index) {
      layer.confirm('确认要删除吗？', {
    	    ...
    	    title:"确定"
		      ...
		      layer.close(flag);
		      httpService.req("DELETE","/mdm/invokeSystem/systemInfo/"+$scope.systemInfoList[index].id, {}, function (result) {
    ...
    $scope.updateStatus = function (index,status) {
		...
		//      	angular.copy($scope.systemInfoList[index], $scope.editParams);
		      	...
		      	httpService.req("PUT","/mdm/invokeSystem/systemInfo/"+$scope.systemInfoList[index].id+"/status",{}, function (result) {
		        	if (result && result.data == 1) {
		          		$rootScope.addAlert("success", "操作成功!");
		          		$scope.querySystemInfoList();
		        	}
