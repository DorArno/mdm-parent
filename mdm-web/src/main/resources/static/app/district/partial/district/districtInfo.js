...
//						$scope.queryEditDistrictTree();
					...
					$scope.queryDistrictTree = function(params) {
						...
						httpService.req('GET', '/mdm/tree/dynamicRoot', params,
								function(result) {
									$scope.html ='';
					...
					$scope.queryDistrictList = function() {
						...
						httpService.req("POST", "/mdm/district/list", {}, function(result) {
					...
					$scope.queryTreeNodes = function(node_id) {
						if (typeof (tree) != "undefined") {
							...
							if("0" != node_id){
								// 按parentId查询节点
								httpService.req('GET', '/mdm/tree/dynamicNodes', {'builder':'districtStaticTreeBuilder','treeType':'district','parentId':node_id},
									function(result) {
...
//						$scope.addParams.parentId = 0;
					...
					$scope.addConfirm = function() {
						if (!$scope.isAdd) {
							...
							//	验证编码是否重复
							httpService.req("POST", "/mdm/district/verifyCode", {}, function(result) {
									var data = result.data;
									if (data != null) {
					...
					$scope.goEdit = function(index) {
						...
						//	查询行政区划信息
						httpService.req("GET", "/mdm/district/"+$scope.districtList[index].id, {}, function(result) {
							var data = result.data;
							if (data != null) {
					...
					$scope.editConfirm = function() {
						if (!$scope.isEdit) {
							...
							httpService.req("PUT", "/mdm/district/"
