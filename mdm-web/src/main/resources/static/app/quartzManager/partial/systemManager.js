...
angular.module('quartzManager')
  .controller('quartzManagerController', function ($scope, $modal, httpService, $rootScope) {
    ...
    $scope.queryInfoList = function () {
    	...
    	$scope.currentPage = 1;
    	httpService.req("GET","/mdm/quartzManager/taskInfos", $scope.queryParams, function (result) {
    ...
    $scope.pauseAllTask = function(){
    	...
    	if(confirm('确认暂停选择任务吗?')){
    	    httpService.req("POST","/mdm/quartzManager/pauseAllTaskInfos",{},function (data) {
    	    	if (data.data == 1) {
    ...
    $scope.resumeAllTask = function(){
    	...
    	if(confirm('确认恢复选择任务吗?')){
    	    httpService.req("POST","/mdm/quartzManager/resumeAllTaskInfos",{},function (data) {
    	    	if (data.data == 1) {
    	            $rootScope.addAlert("success", "操作成功");
    	            $scope.querySystemInfoList();
    	    	}
    ...
    $scope.pauseTask = function(index){
    	...
    	if(confirm('确认暂停任务吗?')){
    	    ...
    	    httpService.req("POST","/mdm/quartzManager/pauseTaskInfos",{},function (data) {
    	    	if (data.data == 1) {
    	            $rootScope.addAlert("success", "操作成功");
    	            $scope.querySystemInfoList();
    	    	}
    ...
    $scope.executeTask = function(index){
    	...
    	if(confirm('确认现在执行任务吗?')){
    		$scope.cronJobInfo.jobName = $scope.systemInfoList[index].triggers[0].jobName;
    		$scope.cronJobInfo.jobGroup = $scope.systemInfoList[index].triggers[0].group;
    	    httpService.req("POST","/mdm/quartzManager/executeTaskInfos",{},function (data) {
    	    	if (data.data == 1) {
    	            $rootScope.addAlert("success", "操作成功");
    	            $scope.querySystemInfoList();
    	    	}
    ...
    $scope.deleteTask = function(index){
    	...
    	if(confirm('确认删除吗?')){
    		$scope.cronJobInfo.jobName = $scope.systemInfoList[index].triggers[0].jobName;
    		$scope.cronJobInfo.jobGroup = $scope.systemInfoList[index].triggers[0].group;
    	    httpService.req("POST","/mdm/quartzManager/deleteTaskInfos",{},function (data) {
    	     if (data.data == 1) {
    	            $rootScope.addAlert("success", "操作成功");
    	            $scope.querySystemInfoList();
    	      }
    ...
    $scope.resumeTask = function(index){
    	...
    	if(confirm('确认恢复任务吗?')){
    		$scope.cronJobInfo.jobName = $scope.systemInfoList[index].triggers[0].jobName;
    		$scope.cronJobInfo.jobGroup = $scope.systemInfoList[index].triggers[0].group;
    	    httpService.req("POST","/mdm/quartzManager/resumeTaskInfos",{},function (data) {
    	    	if (data.data == 1) {
    	            $rootScope.addAlert("success", "操作成功");
    	            $scope.querySystemInfoList();
    	    	}
    ...
    $scope.addTask = function(){
    	 ...
    	 httpService.req("POST","/mdm/quartzManager/addTaskInfos",{},function (data) {
    		 if (data.data == 1) {
    ...
    $scope.editTask = function(){
    	...
    	httpService.req("POST","/mdm/quartzManager/editTaskInfos",{},function (data) {
            if (data.data == 1) {
    ...
    $scope.addConfirm = function () {
      ...
      httpService.req("POST","/mdm/invokeSystem/systemInfos",{},function (data) {
    	  if (data == 1) {
    		  $rootScope.addAlert("success", "操作成功");
    		  $scope.addModal.close();
    		  $scope.querySystemInfoList();
    ...
    $scope.editConfirm = function () {
      ...
      httpService.req("PUT","/mdm/invokeSystem/systemInfos/"+$scope.editParams.id, {}, function (data) {
