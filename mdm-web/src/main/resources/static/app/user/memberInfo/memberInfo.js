...
//				beginDate:$scope.getCurDate(7)
			...
			$scope.queryUserList = function(isReset) {
				...
				httpService.req("POST", "/mdm/user/memberlist", {}, function(
						result) {
			...
			$scope.queryCropCodeStaticDatas = function() {
				...
				httpService.req("GET", "/mdm/staticdata/staticdata", params,
						function(result) {
			...
			$scope.setBackEndUser = function(userId) {
				...
				layer.confirm('确认要设置为后端用户吗？', {
					...
					httpService.req("GET", "/mdm/user/" + userId, {}, function(
							data) {
						if (data != null) {
			...
			$scope.toSetBackEnd = function(userId, corpCode) {
				httpService.req("POST", "/mdm/user/setBackEndUser", {
					'userId' : userId,
					'corpCode' : corpCode
				}, function(data) {
					if (data.resCode == 200) {
