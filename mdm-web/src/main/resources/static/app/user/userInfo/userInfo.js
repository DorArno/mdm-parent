...
//						beginDate:$scope.getCurDate(7)
					...
					$scope.loadOrgTab = function() {
						...
						httpService.req("GET", "/mdm/staticdata", params,
								function(result) {
					...
					$scope.init = function() {
						...
						$scope.systemHash = {};// add by hyz
						httpService.req("GET","/mdm/user/systemInfo", {}, function (result) {
							if (result && result.data) {
								var staticData = result.data;
								$(staticData).each(function(i,e){
									...
									$scope.systemHash[data.sysCode] = data.sysName;
								});
							}
						}, true);
					...
					$scope.updateSetting = function(treeObj,checkable){
						var uSetting = treeObj.getSetting();
						uSetting.checkable = checkable;
						treeObj.updateSetting(uSetting);
						treeObj.refresh();
					};
					// 修改查询组织机构类型
					$scope.setOrgRoleType = function(currentOrgType) {
						// 设置组织树，不可选取
						if("undefined" != typeof orgTree && null != orgTree && orgTree){
							$scope.updateSetting(orgTree, false);
						}
					...
					$scope.esetOrgRoleType = function(currentOrgType) {
						if("undefined" != typeof eorgTree && null != eorgTree && eorgTree){
							$scope.updateSetting(eorgTree, false);
						}
						$('#orgSelect').attr("disabled","disabled");
						document.getElementById("number").innerHTML="";
						document.getElementById("searchKey").value = ""
...
//						$('#editCorpCode').find('input').val(corpCode);
					...
					$scope.setDisplay = function(currentOrgType, radioId){
						...
						if (currentOrgType == 3 || currentOrgType == 2) {
					...
					$scope.loadOrgTree = function() {
						...
						httpService.req('GET', '/mdm/tree/static', params,
								function(result) {
					...
					$scope.loadOrgRoleTree = function(currentOrgType) {
						var isRadio = true;
						var corpcode = "";
						if(currentOrgType == '2' || currentOrgType == '3'){
							isRadio = false;
						...
						httpService.req('GET', '/mdm/tree/static', params,
								function(result) {
									...
									if("undefined" != typeof orgTree && orgTree != null && orgTree){
										orgTree.cancelSelectedNode();
										$scope.updateSetting(orgTree, true);
									}
					...
					$scope.eloadOrgRoleTree = function(currentOrgType) {
						...
						httpService.req('GET', '/mdm/tree/static', params,
								function(result) {
									...
									if("undefined" != typeof eorgTree && eorgTree != null && eorgTree){
										eorgTree.cancelSelectedNode();
										$scope.updateSetting(eorgTree, true);
									}
									$('#orgSelect').removeAttr("disabled");
								}, true);
					};
					...
					$scope.queryUserList = function(reset) {
						...
						httpService.req("POST", "/mdm/user/list", {}, function(
								result) {
					...
					$scope.querySystemInfos = function() {
						httpService.req("GET", "/mdm/invokeSystem/systemInfos",
					...
					$scope.addConfirm = function() {
						...
						// $scope.typeModel;
						//httpService.req("POST", "/mdm/user/verifyPhone", {'cellPhone':$scope.addParams.userBasicsInfoRequest.cellPhone,'id':''}, function(result) {
						var validate_cellPhone = $scope.addParams.userBasicsInfoRequest.cellPhone;
						// 验证编码是否重复
						httpService.req("POST", "/mdm/user/verifyPhone", {'cellPhone':validate_cellPhone, 'id':''}, function(result) {
								var data = result.data;
								if (data != null) {
									if(data == "error"){
...
//													$scope.queryParams.beginDate = $scope.getCurDate(7);
					...
					$scope.proposal = function(userId, cellPhone) {
						layer.confirm('已存在相同手机号的前端用户，要设置为后端用户吗？', {
							...
							httpService.req("POST", "/mdm/user/setBackEndUserForMerge", {}, function(data) {
								if (data.resCode == 200) {
									$scope.closeAdd();
									$scope.cancelAdd();
...
//								$scope.corpCode = $scope.editParams.userBasicsInfoRequest.corpCode;
					...
					$scope.editConfirm = function() {
						...
						// 验证编码是否重复
						/*httpService.req("POST", "/mdm/user/verifyPhone", {'cellPhone':$scope.editParams.userBasicsInfoRequest.cellPhone,'id':$scope.editParams.userBasicsInfoRequest.id}, function(result) {
								var data = result.data;
								if (data != null) {
									$rootScope.addAlert("error", "手机号码重复!");
									$scope.isAdd = false;
...
//													$scope.queryParams.beginDate = $scope.getCurDate(7);
					...
					$scope.goDelete = function(id) {
						layer.confirm('确认要删除吗？', {
				    	    ...
				    	    title:"操作确认"
						      ...
						      layer.close(flag);
						      httpService.req("DELETE", "/mdm/user/"
										+ id, {}, function(
										data) {
					...
					$scope.batchEnable = function() {
						...
						httpService.req("PUT", "/mdm/user/BatchUpdateStatus",
								{}, function(result) {
					...
					$scope.batchDelete = function() {
						...
						layer.confirm('确认要删除吗？', {
				    		...
				    		layer.close(flag);
							httpService.req("PUT", "/mdm/user/BatchDelete", {},
									function(result) {
										if (result.resCode == 200) {
					...
					$scope.resetPassword = function(id) {
						var list = [];
						list.push(id);
						httpService.req("PUT", "/mdm/user/ResetPassword/", {}, function(
								data) {
							if (data.resCode == 200) {
								/*$rootScope.addAlert("success",
