  ...
  .controller('MerchantCrl', function ($scope, $modal, httpService, $rootScope) {
	  ...
	  $scope.systemSourceHash = {};
	    $scope.init = function () {
	    	...
	    	httpService.req("GET","/mdm/staticdata", {tableName:"MerchantOrganization"}, function (result) {
...
//			}, true);
    ...
    $scope.queryFristMerchantType = function () {
        ...
        httpService.req("GET","/mdm/merchant/merchantType", params,function (result) {
          if (result && result.data) {
    ...
    $scope.queryMerchantInfoList = function () {
      httpService.req("GET","/mdm/merchant/merchantInfos", $scope.queryParams, function (result) {
        if (result && result.data) {
    ...
    $scope.addConfirm = function () {
        ...
        httpService.req("POST","/mdm/merchant/addMerchant",{},function (res) {
          if (res.result == "true") {
...
//          httpService.req("POST","/mdm/merchant/merchantInfos",{},function (res) {
//            if (res.result == "true") {
//              $rootScope.addAlert("success", "操作成功");
//              $scope.addModal.close();
//              $scope.queryMerchantInfoList();
...
//        };
    ...
    $scope.goEdit = function (index) {
      ...
      if($scope.editParams.organizationId != '') {
    	  var url = '/mdm/organazation/' + $scope.editParams.organizationId;
    ...
    $scope.editConfirm = function () {
    	...
    	layer.confirm('确认要提交？', {
    	    ...
    	    title:"确认"
		      ...
		      httpService.req("PUT","/mdm/merchant/updateMerchant/"+$scope.editParams.id, {}, function (res) {
...
//    	alert($scope.delParams.id);
        layer.confirm('确认要删除当前商家吗？', {
    	    ...
    	    title:"确认"
		      ...
		      httpService.req("DELETE","/mdm/merchant/deleteMerchant/"+$scope.delParams.id,{},function (res) {
	          if (res.result == "true") {
	        	 $rootScope.addAlert("success", "操作成功!");
		         $scope.queryMerchantInfoList();
	          }
    ...
    $scope.goView = function (index) {
      ...
      $scope.viewParams = $scope.merchantList[index];
        ...
        if($scope.viewParams.organizationId != '') {
            var url = '/mdm/organazation/' + $scope.viewParams.organizationId;
            httpService.req("GET", url, {},
                function(result) {
                    if (result != null && result.data != null ) {
                        var org = result.data;
 ...
 //======================================检验code是否存在===================================================
      ...
      $scope.checkExistParams= function(code){
    	  ...
    	  httpService.req("GET","/mdm/merchant/merchantCheck", Params, function (result) {
    	        if (result!=null && result.data!=null) {
 ...
 //----------------------------------------省市区三级联动数据来源------------------------------------------------
		...
		$scope.queryProvinceList = function() {
			...
			httpService.req("POST", "/mdm/district/districtForSelect", {},
					function(result) {
						if (result != null && result.data != null ) {
	...
	//=====================================================================================================
	    ...
	    $scope.batchUpdateOrgStatus = function (status) {
	    	...
	    	param.flag = status;
	      	httpService.req("PUT","/mdm/merchant/batchUpdateMerchant", {}, function (data) {
	        	if (data.result == "true") {
	          		$rootScope.addAlert("success", "操作成功!");
	          		$scope.queryMerchantInfoList();
	        	}
