angular.module('role').controller(
		...
		function($scope, $modal, httpService, $rootScope) {
			...
			$scope.queryRoleInfoList = function() {
				...
				httpService.req("GET", "/mdm/role/roleInfos", $scope.queryParams, function(result) {
			...
			$scope.querySystemInfo = function() {
				httpService.req("GET", "/mdm/global/systemInfos", {}, function(result) {
					if (result && result.data) {
			...
			$scope.addConfirm = function() {
				...
				httpService.req("POST", "/mdm/role/queryRoleNameExist", {}, function(data) {
					if (data && data.data == 0) {
						httpService.req("POST", "/mdm/role/roleInfos", {}, function(data) {
							if (data && data.data == 1) {
			...
			$scope.goEdit = function(index) {
				...
				httpService.req("GET", "/mdm/role/queryRole/"+$scope.roleList[index].id, {
				}, function(data) {
			...
			$scope.editConfirm = function() {
				...
				httpService.req("PUT", "/mdm/role/roleInfos/" + $scope.editParams.id, {
			...
			$scope.doDel = function(index) {
				layer.confirm('确认要删除吗？', {
					...
					layer.close(flag);
					httpService.req("DELETE", "/mdm/role/roleInfos/" + $scope.roleList[index].id, {
			...
			$scope.addChooseConfirm = function() {
				...
				httpService.req("POST", "/mdm/role/setOperation/" + $scope.role.id, {}, function(
						data) {
					if (data && data.data > 0) {
						$rootScope.addAlert("success", "操作成功!");
			...
			$scope.updateStatus = function(index, status) {
				...
				httpService.req("PUT", "/mdm/role/updateRoleStatus/" + $scope.editParams.id, {},
						function(data) {
							if (data && data.data == 1) {
								$rootScope.addAlert("success", "操作成功!");
								$scope.queryRoleInfoList();
			...
			$scope.queryUserRoleList = function() {
				httpService.req("GET", "/mdm/role/userRoleInfos/" + $scope.role.id,
			...
			$scope.batchUpdateRoleStatus = function(status) {
				...
				httpService.req("PUT", "/mdm/role/batchUpdateRoleStatus", {}, function(data) {
			...
			$scope.batchDeleteRole = function(status) {
				...
				layer.confirm('确认要删除吗？', {
					...
					httpService.req("PUT", "/mdm/role/batchDeleteRole", {}, function(data) {
						if (data && data.data > 0) {
							$rootScope.addAlert("success", "操作成功!");
							$scope.queryRoleInfoList();
						}
