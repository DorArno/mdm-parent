angular.module('operation')
  .controller('OperationCrl', function ($scope, $modal, httpService, $rootScope) {
    ...
    $scope.queryModuleList = function () {
      httpService.req("GET","/mdm/operation/queryModuleList", null, function (result) {
    ...
    $scope.queryOperationList = function () {
      httpService.req("GET","/mdm/operation/operationInfos", $scope.queryParams, function (result) {
        var data = result.data;
        if (data != null) {
    ...
    $scope.addConfirm = function () {
    	if (!$scope.checkEdit($scope.addParams)) {
      	...
      	httpService.req("POST","/mdm/operation/operationInfos",{},function (data) {
        	if (data && data.data == 1) {
    ...
    $scope.editConfirm = function () {
      	...
      	httpService.req("PUT","/mdm/operation/operationInfos/"+$scope.editParams.id, {}, function (data) {
    ...
    $scope.doDel = function (index) {
    	layer.confirm('确认要删除吗？', {
    	    ...
    	    title:"确定"
		      ...
		      layer.close(flag);
		      httpService.req("DELETE","/mdm/operation/operationInfos/"+$scope.systemInfoList[index].id, {}, function (data) {
	        	if (data && data.data > 0) {
	          		$rootScope.addAlert("success", "操作成功!");
	          		$scope.queryOperationList();
	        	}
